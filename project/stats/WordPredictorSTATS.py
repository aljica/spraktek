...
class WordPredictor:
    ...
    def stats(self, filepath):
        ...
        self.user_keystrokes = [0 for i in range(10)] # Number of keystrokes user had to type.
        try:
        ...
        for token in self.tokens:
            ...
            n += 1
            if n%100 == 0:
                print("num words processed", n, "user keystrokes", self.user_keystrokes, "total", self.total_keystrokes)
            ...
            if token not in possible_words:
                possible_words += self.recommend_words() # Add all unigrams
                if token not in possible_words:
                    for i in range(len(self.user_keystrokes)):
                        self.user_keystrokes[i] += len(token) + 1 # User has to type the whole word out.
                    self.index[token] = len(self.index)
                    self.word[len(self.index)] = token
                    self.unigram_count[token] = 1 # Add new word
            ...
            for i in range(10):
                ...
                if token_recommendation_rank <= i+1:
                    # Then we can choose the word right away.
                    self.user_keystrokes[i] += 1 # User keystroke for choosing the recommendation.
                    continue

                user_input=""
                for letter in token:
                    user_input += letter
                    self.user_keystrokes[i] += 1

                    t = self.recommend_words(user_input = user_input, possible_words = possible_words) # Update possible words based on user_input.

                    token_recommendation_rank = t.index(token) + 1
                    if token_recommendation_rank <= i+1:
                        self.user_keystrokes[i] += 1 # For choosing the recommendation.
                        self.words.append(token)
                        break

            self.unigram_count[token] += 1 # Update unigram count. This is a way of learning from user inputs.
            self.words.append(token)

        print(self.user_keystrokes)
        print(self.total_keystrokes)
        self.words = [] # Reset
